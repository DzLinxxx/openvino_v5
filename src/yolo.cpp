...
YOLOV5::YOLOV5(Config config) {
    ...
    this->class_names_path = config.class_names_path;
    this->initialmodel();
}
YOLOV5::~YOLOV5(){}
...
void YOLOV5::initialmodel() {
    std::cout << "Loading model from: " << this->onnx_path << std::endl;
    std::cout << "Loading class names from: " << this->class_names_path << std::endl;
    auto version = ov::get_openvino_version();
    std::cout << "[INFO] OpenVINO Version: " << version.buildNumber << std::endl;
    std::cout << "[INFO] Description: " << version.description << std::endl;
    this->classNames = loadClassNames(this->class_names_path);
    ov::Core core;
...
void YOLOV5::postprocess_img(Mat& frame, float* detections, ov::Shape & output_shape) {
    ...
    for (int i = 0; i < output.size(); i++)
    {
        ...
        int baseLine=0;
        char text[256];
        sprintf(text, "%s %0.1f%%", this->classNames[classId].c_str(), confidence*100);
